...
def Gradient_descent():
    ...
    for i in range(epochs):
        ...
        axs[1].plot(range(i + 1), cost_history, marker='o',color="orange",label='Initial weight')
        axs[1].set_xlabel('Weights',weight='bold')
        ...
        if i % 5 == 0:  # Update the scatter plot after every 10 iterations
            axs[0].clear()
            ...
            axs[0].plot([min(X), max(X)], [min(Y_pred), max(Y_pred)], color='red',alpha=0.5,label='Best fit line',linestyle="--")
