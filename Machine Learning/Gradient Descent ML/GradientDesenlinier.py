...
def Gradient_descent():
    ...
    for i in range(epochs):
        ...
        axs[1].plot(range(i + 1), cost_history, marker='o',label='Initial weight')
        ...
        if i % 1.5 == 0:  # Update the scatter plot after every 10 iterations
            axs[0].clear()
            ...
            axs[0].plot([min(X), max(X)], [min(Y_pred), max(Y_pred)], color='red',alpha=0.3,label='Best fit line')
